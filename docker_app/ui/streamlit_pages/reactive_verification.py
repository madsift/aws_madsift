# In: ui/streamlit_pages/reactive_verification.py

import streamlit as st
from datetime import datetime, timedelta, time, timezone 
from api_client import start_monitoring_job, stop_monitoring_job, load_summary, display_summary
...
def show():
    ...
    """
    st.title("🤖 Plane Automated Content Monitoring")
    st.markdown(
    ...
    subreddit = st.text_input(
        ...
        key="reactive_subreddit"  # ← Unique key
    )
    ...
    sources = st.multiselect(
        ...
        key="reactive_sources"  # ← Unique key
    )
    ...
    if submitted:
        ...
        if not (not query or not sources or start_time_dt >= end_time_dt or ("Reddit" in sources and not subreddit)):
            ...
            st.header("✅ Job Submitted to Backend")
            with st.spinner("Sending job request to the auto_agent_handler..."):
                result = start_monitoring_job(job_payload, "kg-processor")  # ← Fixed: Added result =
            
            if result and result.get("success"):
                job_id = result.get("job_id")
                st.success(f"Backend successfully started job! Job ID: {job_id}")
                
                if "reactive_active_jobs" not in st.session_state:  # ← Unique key
                    st.session_state.reactive_active_jobs = []
                
                job_payload["status"] = "Running"
                job_payload["job_id"] = job_id
                st.session_state.reactive_active_jobs.insert(0, job_payload)  # ← Unique key
                st.rerun()
            else:
                st.error("❌ Backend failed to start the job.")
                st.json(result)
    ...
    if "reactive_active_jobs" in st.session_state and st.session_state.reactive_active_jobs:  # ← Unique key
        for job in st.session_state.reactive_active_jobs:  # ← Unique key
            with st.container(border=True):
                col1, col2, col3 = st.columns([2, 2, 1])
                with col1:
                    st.markdown(f"**Query:** `{job['query']}`")
                    st.caption(f"Subreddit: `{job.get('subreddit', 'N/A')}` | Sources: `{job.get('sources', 'N/A')}`")
                with col2:
                ...
                with col3:
                    st.status(job["status"], state="running")
                    
                    # Action buttons
                    col_stop, col_summary = st.columns(2)
                    with col_stop:
                        if st.button("⏹️ Stop", key=f"stop_{job['job_id']}", use_container_width=True):
                            with st.spinner(f"Stopping job {job['job_id']}..."):
                                stop_result = stop_monitoring_job(job['job_id'], 'kg-processor')
